...
//split the image
void split_image(const CImg<unsigned char> & paper);
...
int main()
{
	...
	for(int image = 6;image <= 6;image++)
	{
		//load file and do edge detect
		string filepath = "./data/" + int_to_string(image) + ".jpg";
...
void split_image(const CImg<unsigned char> & paper){
	int width = paper._width;
	int height = paper._height;
	int w = width / 4;
	int h = height / 4;
	ofstream fout("./out/mynum");
	for(int i = 0;i < 4;i++){
		for(int j = 0;j < 4;j++){
			CImg<unsigned char> temp(w, h, 1, 1, 0);
			for(int x = 0;x < w;x++){
				for(int y = 0;y < h;y++){
					int ww = j*w+x;
					int hh = i*h+y;
					temp(x,y) = 0.2116 * paper(ww, hh, 0) + 0.7152 * paper(ww, hh, 1) + 0.0722 * paper(ww, hh, 2);
				}
			}
			temp.resize(28,28,1,1,3);
			for(int y = 0;y < 28;y++){
				for(int x = 0;x < 28;x++){
					unsigned char t = 255-temp(x,y);
					if(t < 90 || x < 1 || x > 26 || y < 1 || y > 26){
						t = 0;
					}
					temp(x,y) = t;
					fout << (int)t;
			...
			fout << endl;
			temp.save(("./out/"+int_to_string(i*4+j)+".jpg").c_str());
		}
	}
	fout.close();
}
